...
重启 DSH Web 后，在权限选择器中选择 **LLM 替我审批**。插件的浏览器端半身会在菜单项和当前权限按钮前渲染一枚与 DSH 原生权限图标同为 16px、`currentColor` 的终端审批 SVG；它会跟随亮暗主题，不使用文字或 Emoji 充当图标。交互语义与 Codex 的“替我审批”相近：模型审核每次权限升级，但不会获得完全访问权限。包内的 [`dsh/cordis.patch.yml`](dsh/cordis.patch.yml) 会添加该权限预设并挂载插件；不要再手动重复插入同一个插件实例。
